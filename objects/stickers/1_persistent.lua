SMODS.Sticker({
	...
	calculate = function(self, card, context)
		if context.end_of_round and not context.repetition and not context.individual then -- should be main_eval i think but i'm copying from vremade so cry about it
			...
			card_eval_status_text(card, 'extra', nil, nil, nil, {message = localize("k_cost_up"),colour = G.C.RED, delay = 0.45})
			card:set_cost()
		end
	end,
})
...
function SMODS.is_eternal(card, trigger)
	...
	if card.ability.mp_sticker_persistent and not (trigger and trigger.from_sell) then
		ret = true
	end
	return ret
end
...
G.FUNCS.can_sell_card = function(e)
	if e.config.ref_table.ability
	and e.config.ref_table.ability.mp_sticker_persistent then
		if e.config.ref_table:can_sell_card()
		and e.config.ref_table.ability.mp_sell_price <= G.GAME.dollars then
